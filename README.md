...
![Demo GIF](recording_20251227_145429.gif)

## 主な改善点と修正履歴

### 🛠️ 修正・改善
- **システム音声録音の安定化**: `soundcard` ライブラリの導入と、NumPy互換性問題（`numpy.fromstring`）を解決する実行時パッチを実装し、Windowsでのステレオミキサーなし録音を安定化。
- **マルチモニター対応**: Windows環境における高DPIスケーリングやモニター座標系のズレを補正し、全画面/範囲指定録画の精度を向上。
- **GIF出力の高度化**: MP4からの自動高品質変換に加え、3段階（高・中・低）の解像度/FPSプリセットを選択できる機能を追加。
- **UI/UXの刷新**:
  - `qtawesome` を使用したアイコンデザインの導入。
  - Catppuccin Mochaベースのモダンなダークテーマを採用。
  - プログラム起動時のCOMスレッド初期化順序を最適化し、フォルダ参照ボタンのフリーズ問題を解消。
...
- **柔軟な保存形式**
  ...
  - **GIF**: オプションでGIFアニメーションとして保存可能（自動変換・品質設定対応）。
- **モダンなUI / UX**
...
2. **リポジトリのクローンとライブラリのインストール**
   ```bash
   git clone https://github.com/igashira0324/PyRec-OmniCapture.git
   cd screen_recorder
...
2. **設定**:
   ...
   - **保存形式**: 「GIFとしても保存」にチェックを入れると、MP4に加えてGIF形式でも保存されます。品質（質:）も選択可能です。
